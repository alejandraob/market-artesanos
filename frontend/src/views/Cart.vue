  ...
  <div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div v-else class="space-y-6">
      <div v-for="item in cartItems" :key="item.id" class="bg-white rounded-lg shadow p-6 flex items-center gap-6">
        <img :src="item.product?.images?.length ? storageUrl(item.product.images[0]) : '/placeholder.svg'" :alt="item.product?.name" class="w-24 h-24 object-cover rounded" loading="lazy" decoding="async" />
        <div class="flex-grow">
